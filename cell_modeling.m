...
% initialize("...") sets up all parameters and variables necessary and outputs as [P,Y]. Use "circuit" for including the circuit, "oribo" for including the orthogonal ribosomes and "both" for both
% cell"..."ODE(T,Y,P) returns the derivatives with respect to time for each of the variables as an array dY. Fill in "..." with whatever type of ODE you want to simulate.
% How-to:
% To run the ODE, do the following and replace "..." with the ODE you want to simulate:
% [P,Y] = initialize("...");
...
%% Runs all four possibilities to compare
[P,Y] = initialize("n/a");
odeoptions = odeset('NonNegative',[1:length(Y)]);
[T,Y] = ode15s(@(T,Y) cellODE(T,Y,P), [0 2000], Y, odeoptions);
fplot = figure;
figure(fplot.Number); hold('on');
plot(T,Y(:,3))

[P,Y2] = initialize("circuit");
odeoptions = odeset('NonNegative',[1:length(Y2)]);
[T,Y2] = ode15s(@(T,Y2) cellcircuitODE(T,Y2,P), [0 2000], Y2, odeoptions);
plot(T,Y2(:,20))

[P,Y3] = initialize("oribo");
odeoptions = odeset('NonNegative',[1:length(Y3)]);
[T,Y3] = ode15s(@(T,Y3) celloriboODE(T,Y3,P), [0 2000], Y3, odeoptions);
plot(T,Y3(:,3))

[P,Y4] = initialize("both");
odeoptions = odeset('NonNegative',[1:length(Y4)]);
[T,Y4] = ode15s(@(T,Y4) cellbothODE(T,Y4,P), [0 2000], Y4, odeoptions);
plot(T,Y4(:,3))
...
function [P,Y] = initialize(options)
    ...
    %   Arguments "options" describes what to add ("circuit" or "ortho")
    ...
    P = [s_e;phi_e;v_T;v_E;k_T;k_E;w_T;w_E;w_H;w_R;w_r;o_T;o_E;o_H;o_R;o_r;k_H;h_H;d_m;d_r;b_T;b_E;b_H;b_R;b_r;u_T;u_E;u_H;u_R;u_r;d_p;n_T;n_E;n_H;n_R;gamma_max;k_gamma;M_proteome];
    ...
    if options == "circuit" % Add circuit variables if necessary
        ...
        m_Y = 10; % mRNAs for circuit genes
        c_Y = 10; % translation complexes for circuit genes
        p_Y = 10; % circuit proteins
        Y = [Y;m_Y;c_Y;p_Y];
    end
    if options == "oribo" % Add orthogonal ribosome variables if necessary
        %% Variables for circuit genes
        r_O = 10; % 16S rRNA for orthogonal ribosomes
...
function dY = cellODE(T,Y,P)
    ...
    dm_H = w_H/(1+(p_H/k_H)^h_H)*E_cell/(E_cell+o_H) + gamma*c_H/n_H - b_H*R_h*m_H + u_H*c_H - (d_m+lambda)*m_H; % Supp. eqn 4, including correct R factor for regulation of host proteins
    ...
    dR_h = b_r*p_R*r_R - u_r*R_h  - (d_p+lambda)*R_h + gamma*c_T/n_T+gamma*c_E/n_E+gamma*c_H/n_H+gamma*c_R/n_R + u_T*c_T+u_E*c_E+u_R*c_R+u_H*c_H - (b_T*R_h*m_T+b_E*R_h*m_E+b_R*R_h*m_R+b_H*R_h*m_H); % Supp. eqn 10, with sign change on the sum to fix error
...
function dY = cellcircuitODE(T,Y,P)
    ...
    dp_R = gamma*c_R/n_R - (d_p+lambda)*p_R - b_r*p_R*r_R + u_r*R_h; % Supp. eqn 9
    dr_R = w_r*E_cell/(E_cell+o_r) - b_r*p_R*r_R + u_r*R_h - (d_r+lambda)*r_R; % Supp. eqn 8
    dR_h = b_r*p_R*r_R - u_r*R_h  - (d_p+lambda)*R_h + gamma*(c_T/n_T+c_E/n_E+c_H/n_H+c_R/n_R+c_Y/n_Y) + u_T*c_T+u_E*c_E+u_R*c_R+u_H*c_H+u_Y*c_Y - (b_T*R_h*m_T+b_E*R_h*m_E+b_R*R_h*m_R+b_H*R_h*m_H+b_Y*R_h*m_Y); % Supp. eqn 13, with sign change on the sum to fix error
...
function dY = cellbothODE(T,Y,P)
    ...
    dr_R = w_r*E_cell/(E_cell+o_r) - b_r*p_R*r_R + u_r*R_h - (d_r+lambda)*r_R; % Supp. eqn 8
    dr_O = w_O*E_cell/(E_cell+o_O) - b_O*p_R*r_O + u_O*R_O - (d_O+lambda)*r_O; % Supp. eqn 15
    dR_O = b_O*p_R*r_O - u_O*R_O - (d_p+lambda)*R_O + gamma*c_Y/n_Y - b_Y*R_O*m_Y + u_Y*c_Y; % Supp. eqn 17, with the (corrected) modifications from supp. eqn 13 to allow translation of circuit genes
    dR_h = b_r*p_R*r_R - u_r*R_h  - (d_p+lambda)*R_h + gamma*(c_T/n_T+c_E/n_E+c_H/n_H+c_R/n_R) + u_T*c_T+u_E*c_E+u_R*c_R+u_H*c_H - (b_T*R_h*m_T+b_E*R_h*m_E+b_R*R_h*m_R+b_H*R_h*m_H); % Supp. eqn 10, with sign change on the sum to fix error
